import numpy as np
import glob, os, sys, json, inspect
...
for par in psr.fitpars:
    if par == "Offset":
        ...
        if args.sample_cos:
            ...
            if args.psr_name == "J1640+2224" and args.datarelease == "5yr":
                tm_param_dict["COSI"] = {
...
if args.zero_start:
    ...
    for n in range(nwalkers):
        ...
        for p in pta.params:
            ...
            if p.name == "J0709+0458_timing_model_ECC":
                print(p, x0_list[-1])
...
"""
for idx, par in enumerate(pta.params):
    print(par)
    print(start[:, idx])
print(np.shape(pta.params))
print(
    [
        start[:, idx]
        for idx, par in enumerate(pta.params)
        if par.name == "J0709+0458_timing_model_ECC"
    ]
)
...
nsteps = N
save_point = 200
maxiter = 1e5  # (Default is 1e4)
nchains = 1
"""
light_mode (bool): If True (default is False) 
    then no expansions are performed after the tuning phase. 
    This can significantly reduce the number of log likelihood evaluations
     but works best in target distributions that are apprroximately Gaussian.
"""
light_mode = True
    ...
    if N_cores > 1:
        ...
        while taken_samples < nsteps:
            if taken_samples != 0.0:
                start = prev_samples[-1]
            sampler = zeus.EnsembleSampler(
                nwalkers, ndim, get_lnpost, maxiter=int(maxiter), light_mode=light_mode
            )
            sampler.run_mcmc(start, save_point)
            # chain = sampler.get_chain(flat=True, discard=0.5)
            chain_all = sampler.get_chain()
