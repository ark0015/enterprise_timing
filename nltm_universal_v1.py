...
parser.add_argument('--psrlist', required=True,
    help='name of pulsar used for search')
...
parser.add_argument('--tm_prior', choices=['uniform','bounded'], default='uniform',
                    help='Use either uniform or bounded for ephemeris modeling? (DEFAULT: uniform)')

parser.add_argument('--ephem',default='DE436',
                    help='Ephemeris option (DEFAULT: DE436)')

parser.add_argument('--white_var', default=True, help='Vary the white noise? (DEFAULT: TRUE)')
parser.add_argument('--red_var', default=True, help='Vary the white noise? (DEFAULT: TRUE)')

parser.add_argument('--resume', default=False, help='Whether to resume the chains.')

parser.add_argument('--N', default=int(1e6),help='Number of samples (DEFAULT: 1e6)')

parser.add_argument('--coefficients', default=False,
 help='Whether to keep track of linear components. (DEFAULT: False')

parser.add_argument('--tm_var', default=True,
 help='Whether to vary timing model. (DEFAULT: True')

parser.add_argument('--tm_linear', default=False,
 help='Whether to use only the linear timing model. (DEFAULT: False')

parser.add_argument('--nltm_plus_ltm', default=False,
 help='Whether to use non-linear plus linear timing model variations. (DEFAULT: False')

parser.add_argument('--exclude', default=True,
 help='Whether to exclude non-linear parameters from ' +\
  'linear timing model variations, or only include some parameters in linear timing model. (DEFAULT: True')

parser.add_argument('--writeHotChains', default=True,
 help='Whether to write out the parallel tempering chains. (DEFAULT: True')
parser.add_argument('--reallyHotChain', default=False,
 help='Whether to include a really hot chain in the parallel tempering runs. (DEFAULT: False')
...
if args.nltm_plus_ltm:
    ...
    outdir = current_path + "/{}/chains/{}/".format(psrlist[0],args.datarelease) + psrlist[0] +\
    "_{}_{}_tm_{}/".format("_".join(args.tm_prior.split('-')),args.ephem,args.run_num)
...
if len(psrs) == 0:
    raise ValueError('{} does not exist in {} datarelease.'.format(psrlist[0],args.datarelease))
tm_params_nodmx = []
ltm_exclude_list=[]
for psr in psrs:
...
psampler.sample(x0, N, SCAMweight=30, AMweight=15, DEweight=50,
    writeHotChains=args.writeHotChains,hotChain=args.reallyHotChain)