...
def get_chain_tmparam_dict(chaindir_list,burn=None):
    ...
    chain_dict = {}
    chain_dict['uniform'] = {}
    chain_dict['uniform']['px_priors'] = {}
    chain_dict['uniform']['other'] = {}

    chain_dict['bounded'] = {}
    chain_dict['bounded']['px_priors'] = {}
        ...
        if 'uniform' in name.split('_') and not 'timing' in name.split('_'):
            if 'PX' in name.split('_'):
                if dataset not in chain_dict['uniform']['px_priors'].keys():
                    chain_dict['uniform']['px_priors'][dataset] = defaultdict(list)
                chain_dict['uniform']['px_priors'][dataset]['chains'].append(chain[burn:])
                chain_dict['uniform']['px_priors'][dataset]['tmparams'].append(tmp_tmparam)
            else:
            ...
            if 'PX' in name.split('_'):
                if dataset not in chain_dict['bounded']['px_priors'].keys():
                    chain_dict['bounded']['px_priors'][dataset] = defaultdict(list)
                chain_dict['bounded']['px_priors'][dataset]['chains'].append(chain[burn:])
                chain_dict['bounded']['px_priors'][dataset]['tmparams'].append(tmp_tmparam)
            else:
    ...
    for prior, prior_dict in chain_dict.items():
        for px_prior, px_prior_dict in prior_dict.items():
            for dataset, dataset_dict in px_prior_dict.items():
                for i, chain in enumerate(chain_dict[prior][px_prior][dataset]['chains']):
                    print(np.shape(chain))
                    for j,par in enumerate(chain_dict[prior][px_prior][dataset]['tmparams'][i]):
                        if par.split('_')[0] in [psr.name for psr in psrs]: 
                            if list(par)[0] != 'J':
                                par = 'J' + par
                        if par in comb_chain_dict.keys():
            ...
            for tmparams,chains in zip(chain_dict[prior][px_prior][dataset]['tmparams'],
                ...
                if j == 0:
                    if not rescaled_chain_dict:
                        rescaled_chain_dict = get_rescaled_chain_dict([chains],[tmparams],pardict,dataset)
                    else:
                        rescaled_chain_dict_comb = get_rescaled_chain_dict([chains],[tmparams],pardict,dataset,
...
def get_rescaled_chains_as_core_list(chain_dict,label,burn=0):
    ...
    for key,val in chain_dict.items():
        if np.shape(val) == max_size:
            ...
            print('Exluding ',key, ' because its shape {} doesnt match others'.format(np.shape(val)))
    ...
    for i in range(num_chains):
        ...
        core_list.append(co.Core(label='{}'.format(label),chain=stacked_chains.T,
              params=param_list,burn=burn))
    return core_list

def plot_common_chains(core_list,chaindir_list,priors,px_priors,dataset,plot_kwargs={},misc_kwargs={}):
